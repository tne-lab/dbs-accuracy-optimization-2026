...
fileConfigs = {'Results_Continuous_prob4_RawRT.mat', 'prob4';
               'Results_Continuous_prob6_RawRT.mat', 'prob6'};
...
currentIter = 4200;
iterField = sprintf('Iter%d', currentIter);
...
title(t, 'Algorithm Trial level regret', 'FontSize', 32, 'FontWeight', 'bold');
...
for f = 1:2
    ...
    patch([0 currentIter currentIter 0], [0 0 10 10], [0.46 0.67 0.18], ...
    ...
    for a_idx = 1:length(algosToPlot)
        ...
        pld = 'Iterations'; if isfield(res, algoName) && ~isfield(res.(algoName), pld), pld = 'Day'; end
        
        if isfield(res, algoName) && isfield(res.(algoName).(pld), iterField)
            curve = res.(algoName).(pld).(iterField).Trial_Param_Regret_Mean_Curve;
    ...
    if ~isempty(labelData)
        ...
        for iter = 1:100 % Iterate to find equilibrium
    ...
    ylim([-2, 70]); 
    xlim([0, currentIter * 1.2]); % Wide margin to prevent text clipping
    set(ax, 'YTick', 0:10:70);
    
    % Interpretive Callouts
    text(100, 67, '\uparrow High Information Loss', 'FontSize', 10, 'Color', [0.6 0.2 0.2], 'FontAngle', 'italic');
    text(100, 3, '\downarrow Target Achieved', 'FontSize', 10, 'Color', [0.2 0.5 0.2], 'FontAngle', 'italic');
end