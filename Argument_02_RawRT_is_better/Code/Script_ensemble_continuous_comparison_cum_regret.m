%% ============================================================
% Pure Day 7 & Continuous Cumulative Regret Plot (Aesthetic Polish)
...
ensBF_raw  = dataLoadEns.results_accumulator.noalgorithm.Day.Day7.Trial_Param_Regret_Mean_Curve;
ensUCB_raw = dataLoadEns.results_accumulator.UCB.Day.Day7.Trial_Param_Regret_Mean_Curve;
conBF_raw  = dataLoadCon.results_accumulator.noalgorithm.Iterations.Iter4200.Trial_Param_Regret_Mean_Curve;
conUCB_raw = dataLoadCon.results_accumulator.UCB.Iterations.Iter4200.Trial_Param_Regret_Mean_Curve;

% Calculate Cumulative Regret using cumsum and scale to ms (* 1000)
conBF  = cumsum(conBF_raw) * 1000;
conUCB = cumsum(conUCB_raw) * 1000;
ensBF  = cumsum(ensBF_raw) * 1000;
ensUCB = cumsum(ensUCB_raw) * 1000;
...
p1 = stairs(ax, conBF,  'LineWidth', 2.0, 'Color', [colorBF, 0.85],  'LineStyle', '-',  'DisplayName', 'BF - Continuous');
p2 = stairs(ax, conUCB, 'LineWidth', 2.0, 'Color', [colorUCB, 0.85], 'LineStyle', '-',  'DisplayName', 'UCB1 - Continuous');

% Plot Day 7 curves cleanly by supplying Y first, then shifting its timeline
p3 = stairs(ax, ensBF,  'LineWidth', 1.4, 'Color', [colorBF, 0.60],  'LineStyle', '-.', 'DisplayName', 'BF - Ensemble');
p4 = stairs(ax, ensUCB, 'LineWidth', 1.4, 'Color', [colorUCB, 0.60], 'LineStyle', '-.', 'DisplayName', 'UCB1 - Ensemble');
...
all_curves = [conBF(:); conUCB(:); ensBF(:); ensUCB(:)];
...
% Labels and Fonts updated for Cumulative Regret
xlabel(ax, 'Trial number', 'FontSize', 15, 'FontWeight', 'normal', 'FontName', 'Arial');
ylabel(ax, 'Cumulative Regret for Problem Size 8 in ms', ...
    'FontSize', 17, 'FontWeight', 'bold', 'FontName', 'Arial', 'Interpreter', 'tex');
...
% Changed to 'NorthWest' if the curves climb up and clear the bottom-left area
lgd = legend(ax, [p1, p2, p3, p4], 'Location', 'NorthWest', 'NumColumns', 1, 'FontSize', 11.5, 'FontName', 'Arial');
